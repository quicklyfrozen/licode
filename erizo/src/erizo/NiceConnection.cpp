...
namespace erizo {
  ...
  void cb_component_state_changed(NiceAgent *agent, guint stream_id,
    ...
    printf("cb_component_state_changed %u component %u\n", state, component_id);
    if (state == NICE_COMPONENT_STATE_READY) {
      NiceConnection *conn = (NiceConnection*) user_data;
      conn->updateIceState(READY);
    } else if (state == NICE_COMPONENT_STATE_FAILED) {
      printf("Ice Component failed %u \n", component_id);
      NiceConnection *conn = (NiceConnection*) user_data;
  ...
  void NiceConnection::init() {
    ...
    for (int i = 1; i <= iceComponents_; i++){
      nice_agent_attach_recv(agent_, 1, i, g_main_loop_get_context(loop_),
          cb_nice_recv, this);
    }
  ...
  bool NiceConnection::setRemoteCandidates(
    ...
    for (int currentComp = 1; currentComp<=iceComponents_; currentComp++){
      ...
      for (unsigned int it = 0; it < candidates.size(); it++) {
        ...
        if (cinfo.componentId != currentComp)
          continue;
      ...
      nice_agent_set_remote_candidates(agent_, (guint) 1, currentComp, candList);
    }
